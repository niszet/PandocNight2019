...
* Pandocのマニュアルを読むか、R Markdownのマニュアルが便利（最低限がまとまっているので）

* 書籍はコレでしょ！(Markdownの本全然ない…）
...
* スタイルの設定が大変だが、一度テンプレートファイルを作成してしまえば使いまわしがきくので時間をかける価値はある。
...
そのため、スタイルの割り当てで対応するスタイルがないとうまくスタイルをあてられない。
